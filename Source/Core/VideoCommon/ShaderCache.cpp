...
ShaderCache::CompilePixelUberShader(const UberShader::PixelShaderUid& uid) const
{
  ...
  std::string name = fmt::format("Pixel UberShader for {} texgens{}{}{}",
                                 data->num_texgens,
                                 data->early_depth ? ", early depth" : "",
                                 data->per_pixel_depth ? ", per-pixel depth" : "",
                                 data->no_dual_src ? ", no dual-source blending" : "");
...
std::optional<AbstractPipelineConfig> ShaderCache::GetGXPipelineConfig(const GXPipelineUid& config_in)
{
...
static GXUberPipelineUid ApplyDriverBugs(const GXUberPipelineUid& in)
{
  ...
  if (g_ActiveConfig.backend_info.bSupportsFramebufferFetch)
  {
    ...
    out.blending_state.blendenable = false;
    out.blending_state.usedualsrc = false;
    out.blending_state.logicopenable = false;
    ...
    UberShader::EnumeratePixelShaderUids([&](const UberShader::PixelShaderUid& puid) {
      ...
      EnumerateGeometryShaderUids([&](const GeometryShaderUid& guid) {
        ...
        if (g_ActiveConfig.backend_info.bSupportsDynamicVertexLoader)
        {
          ...
          // compiled shaders based on some subset of the pipeline state.  Some examples:
          // MacOS Metal:
          ...
          //  - Nvidia Kepler: Acts just like D3D11
...
ShaderCache::GetTextureDecodingShader(TextureFormat format,
                                      const TextureConversionShaderTiled::DecodingShaderInfo* info,
                                      TLUTFormat palette_format)
{
  if (info->palette_size == 0)
    palette_format = TLUTFormat::IA8;  // Reduce unnecessary copies
  ...
  std::string shader_source =
      TextureConversionShaderTiled::GenerateDecodingShader(info, palette_format, APIType::OpenGL);
  if (shader_source.empty())
  {
