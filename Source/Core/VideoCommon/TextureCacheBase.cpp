                                          ...
                                          TLUTFormat palette_format)
{
  ...
  const AbstractShader* shader =
      g_shader_cache->GetTextureDecodingShader(format, info, palette_format);
  if (!shader)
    return false;
  ...
  g_renderer->DispatchComputeShader(shader,
                                    info->group_size_x, info->group_size_y, 1,
