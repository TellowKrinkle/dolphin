...
ATTR_TARGET DOLPHIN_FORCE_INLINE static __m256 ApplyMatrixYMM(__m256 v, __m256 m0, __m256 m1,
                                                              __m256 m2, __m256 m3)
{
  __m256 output = _mm256_mul_ps(_mm256_shuffle_ps(v, v, _MM_SHUFFLE(0, 0, 0, 0)), m0);
#ifdef USE_FMA
  output = _mm256_fmadd_ps(_mm256_shuffle_ps(v, v, _MM_SHUFFLE(1, 1, 1, 1)), m1, output);
  output = _mm256_fmadd_ps(_mm256_shuffle_ps(v, v, _MM_SHUFFLE(2, 2, 2, 2)), m2, output);
  output = _mm256_fmadd_ps(_mm256_shuffle_ps(v, v, _MM_SHUFFLE(3, 3, 3, 3)), m3, output);
#else
  output = _mm256_add_ps(output, _mm256_mul_ps(_mm256_shuffle_ps(v, v, 0x55), m1));
  output = _mm256_add_ps(output, _mm256_mul_ps(_mm256_shuffle_ps(v, v, 0xAA), m2));
  output = _mm256_add_ps(output, _mm256_mul_ps(_mm256_shuffle_ps(v, v, 0xFF), m3));
#endif
  return output;
}
...
#ifdef USE_FMA
  output = _mm256_fmadd_ps(_mm256_shuffle_ps(vertex, vertex, 0x00), pos0, output);
  output = _mm256_fmadd_ps(_mm256_shuffle_ps(vertex, vertex, 0x55), pos1, output);
  if constexpr (PositionHas3Elems)
    output = _mm256_fmadd_ps(_mm256_shuffle_ps(vertex, vertex, 0xAA), pos2, output);
#else
  output = _mm256_add_ps(output, _mm256_mul_ps(_mm256_shuffle_ps(vertex, vertex, 0x00), pos0));
  output = _mm256_add_ps(output, _mm256_mul_ps(_mm256_shuffle_ps(vertex, vertex, 0x55), pos1));
  if constexpr (PositionHas3Elems)
    output = _mm256_add_ps(output, _mm256_mul_ps(_mm256_shuffle_ps(vertex, vertex, 0xAA), pos2));
#endif
  return ApplyMatrixYMM(output, proj0, proj1, proj2, proj3);
}
...
{
#if defined(USE_SSE)
  Vector output = _mm_mul_ps(_mm_shuffle_ps(v, v, _MM_SHUFFLE(0, 0, 0, 0)), m0);
#ifdef USE_FMA
  output = _mm_fmadd_ps(_mm_shuffle_ps(v, v, _MM_SHUFFLE(1, 1, 1, 1)), m1, output);
  output = _mm_fmadd_ps(_mm_shuffle_ps(v, v, _MM_SHUFFLE(2, 2, 2, 2)), m2, output);
  output = _mm_fmadd_ps(_mm_shuffle_ps(v, v, _MM_SHUFFLE(3, 3, 3, 3)), m3, output);
#else
  output = _mm_add_ps(output, _mm_mul_ps(_mm_shuffle_ps(v, v, _MM_SHUFFLE(1, 1, 1, 1)), m1));
  output = _mm_add_ps(output, _mm_mul_ps(_mm_shuffle_ps(v, v, _MM_SHUFFLE(2, 2, 2, 2)), m2));
  output = _mm_add_ps(output, _mm_mul_ps(_mm_shuffle_ps(v, v, _MM_SHUFFLE(3, 3, 3, 3)), m3));
#endif
  return output;
#elif defined(USE_NEON)
...
#if defined(USE_FMA)
  output = _mm_fmadd_ps(_mm_shuffle_ps(vertex, vertex, 0x00), pos0, output);
  output = _mm_fmadd_ps(_mm_shuffle_ps(vertex, vertex, 0x55), pos1, output);
  if constexpr (PositionHas3Elems)
    output = _mm_fmadd_ps(_mm_shuffle_ps(vertex, vertex, 0xAA), pos2, output);
#elif defined(USE_SSE)
  output = _mm_add_ps(output, _mm_mul_ps(_mm_shuffle_ps(vertex, vertex, 0x00), pos0));
  output = _mm_add_ps(output, _mm_mul_ps(_mm_shuffle_ps(vertex, vertex, 0x55), pos1));
  if constexpr (PositionHas3Elems)
    output = _mm_add_ps(output, _mm_mul_ps(_mm_shuffle_ps(vertex, vertex, 0xAA), pos2));
#elif defined(USE_NEON)
...
ATTR_TARGET static void TransformVertices(void* output, const void* vertices, u32 stride, int count)
{
  ...
  LoadTransposedYMM(VertexShaderManager::constants.projection.data(), proj0, proj1, proj2, proj3);
  ...
  LoadTransposed(VertexShaderManager::constants.projection.data(), proj0, proj1, proj2, proj3);
  LoadTransposedPos(&xfmem.posMatrices[idx * 4], pos0, pos1, pos2, pos3);
...
#else
  Vector part3 = _mm_shuffle_ps(part2, part2, _MM_SHUFFLE(1, 1, 1, 1));
#endif
  Vector part4 = _mm_shuffle_ps(part2, part2, _MM_SHUFFLE(3, 3, 3, 3));
...
ATTR_TARGET static bool CullVertices(const CPUCull::TransformedVertex* transformed, int count)
{
  switch (Primitive)
  {
