...
float4 load_input_float4_ubyte4(uint attr_offset) {{
  return float4(load_input_uint4_ubyte4(attr_offset)) / 255.0f;
}}

float3 load_input_float3_float3(uint attr_offset) {{
  uint offset = GetOffset(attr_offset);
...
float4 load_input_float4_rawpos(uint attr_offset) {{
  uint components = attr_offset >> 16;
  uint offset = GetOffset(attr_offset & 0xffff);
  if (components < 3)
...
float3 load_input_float3_rawtex(uint attr_offset) {{
  uint components = attr_offset >> 16;
  uint offset = GetOffset(attr_offset & 0xffff);
  if (components < 2)
  ...
  {
    out.Write("ATTRIBUTE_LOCATION({}) in float4 in_rawpos;\n", SHADER_POSITION_ATTRIB);
    out.Write("ATTRIBUTE_LOCATION({}) in uint4 in_posmtx;\n", SHADER_POSMTX_ATTRIB);
    out.Write("ATTRIBUTE_LOCATION({}) in float3 in_rawnormal;\n", SHADER_NORMAL_ATTRIB);
    out.Write("ATTRIBUTE_LOCATION({}) in float3 in_rawtangent;\n", SHADER_TANGENT_ATTRIB);
    out.Write("ATTRIBUTE_LOCATION({}) in float3 in_rawbinormal;\n", SHADER_BINORMAL_ATTRIB);
    out.Write("ATTRIBUTE_LOCATION({}) in float4 in_rawcolor0;\n", SHADER_COLOR0_ATTRIB);
    out.Write("ATTRIBUTE_LOCATION({}) in float4 in_rawcolor1;\n", SHADER_COLOR1_ATTRIB);
    for (int i = 0; i < 8; ++i)
      out.Write("ATTRIBUTE_LOCATION({}) in float3 in_rawtex{};\n", SHADER_TEXTURE0_ATTRIB + i, i);
  }
                                   ...
                                   u32 num_texgen, ShaderCode& out)
{
  ...
  if (host_config.backend_dynamic_vertex_loader)
  {
    out.Write("        int tmp = int(load_input_float3_rawtex(vertex_offset_rawtex[texgen / 4]"
              "[texgen % 4]).z);\n"
              "\n");
  }
  else
  {
    out.Write("        // This is messy, due to dynamic indexing of the input texture coordinates.\n"
              ...
              "        switch (texgen) {{\n");
    for (u32 i = 0; i < num_texgen; i++)
      out.Write("        case {}u: tmp = int(in_rawtex{}.z); break;\n", i, i);
    out.Write("        }}\n"
              "\n");
  }
                                ...
                                std::string_view stored_type, std::string_view offset_name)
{
  if (host_config.backend_dynamic_vertex_loader)
  {
    code.Write("{:{}}{} {} = load_input_{}_{}(vertex_offset_{});\n", "", indent, shader_type, name,
               shader_type, stored_type, offset_name.empty() ? name : offset_name);
  }
  else
  {
    code.Write("{:{}}{} {} = in_{};\n", "", indent, shader_type, name, name);
  }
}
