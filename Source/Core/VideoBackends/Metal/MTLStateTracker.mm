...
void Metal::StateTracker::PrepareRender()
{
  ...
  if (is_gx)
  {
    ...
    if (!m_flags.has_gx_vs_uniform)
    {
      ...
      // Vertex buffer is only used for per-pixel lighting, so it'll be on all or no fragment shaders, no need to track separately
...
{
  PrepareCompute();
  [m_current_compute_encoder dispatchThreadgroups:MTLSizeMake(groups_x, groups_y, groups_z)
