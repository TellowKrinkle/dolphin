...
Metal::StateTracker::StateTracker()
  : m_backref(std::make_shared<Backref>(this))
{
...
void* Metal::StateTracker::Preallocate(UploadBuffer buffer_idx, size_t amt)
{
  ...
  if (!g_features.unified_memory && needs_upload)
  {
    ...
    {
      id<MTLBlitCommandEncoder> enc = GetUploadEncoder();
...
Metal::StateTracker::Map Metal::StateTracker::CommitPreallocation(UploadBuffer buffer_idx, size_t amt)
{
  BufferPair& buffer = m_upload_buffers[static_cast<int>(buffer_idx)];
  size_t pos = buffer.usage.Allocate(m_current_draw, amt);
...
MTLRenderPassDescriptor* Metal::StateTracker::GetRenderPassDescriptor(Framebuffer* framebuffer, MTLLoadAction load_action)
{
  AbstractTextureFormat depth_fmt = framebuffer->GetDepthFormat();
...
{
  EndRenderPass();
  m_current_render_encoder = MRCRetain([GetRenderCmdBuf() renderCommandEncoderWithDescriptor:descriptor]);
  if (!g_features.unified_memory)
  ...
  static_assert(std::is_trivially_copyable<decltype(m_current)>::value, "Make sure we can memset this");
  ...
  m_current.viewport = { 0.f, 0.f, static_cast<float>(m_current.width), static_cast<float>(m_current.height), 0.f, 1.f };
...
void Metal::StateTracker::SetPipeline(const Pipeline* pipe)
{
  if (pipe != m_state.render_pipeline)
  {
    ...
    if (pipe->Usage() == AbstractPipelineUsage::Uber
        && 0 != memcmp(&pipe->UberVertexAttrs(), &m_state.uber_vertex_attrs, sizeof(UberShaderVertexAttributes)))
    {
      m_flags.has_gx_vs_uber_attrs = false;
      memcpy(&m_state.uber_vertex_attrs, &pipe->UberVertexAttrs(), sizeof(UberShaderVertexAttributes));
    }
  }
}
...
void Metal::StateTracker::SetViewport(float x, float y, float width, float height, float near_depth, float far_depth)
{
...
{
  m_flags.has_viewport = 0 == memcmp(&m_state.viewport, &m_current.viewport, sizeof(m_current.viewport));
}
...
void Metal::StateTracker::SetBBoxBuffer(id<MTLBuffer> bbox, id<MTLFence> upload, id<MTLFence> download)
{
  m_state.bbox = bbox;
...
void Metal::StateTracker::PrepareRender()
{
  ...
  bool is_uber = pipe->Usage() == AbstractPipelineUsage::Uber;
  ...
  if (!m_flags.has_pipeline)
  {
    ...
    MTLDepthClipMode clip = is_gx && g_ActiveConfig.backend_info.bSupportsDepthClamp ? MTLDepthClipModeClamp : MTLDepthClipModeClip;
    if (clip != m_current.depth_clip_mode)
    {
      m_current.depth_clip_mode = clip;
      [enc setDepthClipMode:clip];
    }
    if (is_gx && m_state.bbox_upload_fence && !m_flags.bbox_fence)
    {
  ...
  if (u8 dirty = m_dirty_samplers & pipe->GetSamplers())
  {
    ...
    [enc setFragmentSamplerStates:m_state.samplers + range.location
                     lodMinClamps:m_state.sampler_min_lod
                     lodMaxClamps:m_state.sampler_max_lod
                        withRange:range];
  }
  if (is_gx)
  {
    // GX draw
    if (!m_flags.has_gx_vs_uniform && (pipe->UsesVertexBuffer(1) || pipe->UsesFragmentBuffer(1)))
    {
      ...
      ADDSTAT(g_stats.this_frame.bytes_uniform_streamed, Align(sizeof(VertexShaderConstants), AlignMask::Uniform));
    }
    if (!m_flags.has_gx_ps_uniform && pipe->UsesFragmentBuffer(0))
    {
      ...
      ADDSTAT(g_stats.this_frame.bytes_uniform_streamed, Align(sizeof(PixelShaderConstants), AlignMask::Uniform));
    }
    if (!m_flags.has_gx_vs_uber_attrs && is_uber)
    {
      m_flags.has_gx_vs_uber_attrs = true;
      [enc setVertexBytes:&m_state.uber_vertex_attrs length:sizeof(UberShaderVertexAttributes) atIndex:2];
    }
  }
  else
  {
...
void Metal::StateTracker::PrepareCompute()
{
  ...
  {
    m_flags.has_pipeline = true;
    [enc setComputePipelineState:m_state.compute_pipeline->GetComputePipeline()];
  }
  if (!m_flags.has_compute_texture)
  {
  ...
  if (!m_flags.has_utility_vs_uniform)
  {
    m_flags.has_utility_vs_uniform = true;
...
void Metal::StateTracker::DispatchComputeShader(u32 groups_x, u32 groups_y, u32 groups_z)
{
  PrepareCompute();
  // TODO: Don't hardcode threadgroup shape
  [m_current_compute_encoder dispatchThreads:MTLSizeMake(groups_x, groups_y, groups_z)
                       threadsPerThreadgroup:MTLSizeMake(8, 8, 1)];
}

void Metal::StateTracker::ResolveTexture(id<MTLTexture> src, id<MTLTexture> dst, u32 layer, u32 level)
{
  EndRenderPass();
  auto color0 = [[m_resolve_pass_desc colorAttachments] objectAtIndexedSubscript:0];
  ...
  id<MTLRenderCommandEncoder> enc = [GetRenderCmdBuf() renderCommandEncoderWithDescriptor:m_resolve_pass_desc];
  [enc endEncoding];
}
