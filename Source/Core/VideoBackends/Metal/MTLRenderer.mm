...
{
  std::string msl = Util::TranslateShaderToMSL(stage, source);
  if (msl.empty())
  {
...
std::unique_ptr<AbstractShader> Metal::Renderer::CreateShaderFromMSL(
...
{ @autoreleasepool {
  ...
  if (!name.empty())
    ...
    [fn setLabel:[NSString stringWithFormat:GenericShaderName(stage), m_shader_counter[static_cast<u32>(stage)]++]];
  if (stage == ShaderStage::Compute)
  {
    ...
    MRCOwned<id<MTLComputePipelineState>> pipeline = MRCTransfer([g_device
    ...
    return std::make_unique<ComputePipeline>(stage, reflection, std::move(msl), std::move(fn), std::move(pipeline));
  }
...
{ @autoreleasepool {
  ...
  g_state_tracker->DispatchComputeShader(groupsize_x, groupsize_y, groupsize_z, groups_x, groups_y, groups_z);
}}
