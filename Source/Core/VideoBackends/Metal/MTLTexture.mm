  ...
  const MathUtil::Rectangle<int>& dst_rect, u32 dst_layer, u32 dst_level)
{
  ...
  [blit copyFromTexture:msrc
           ...
           sourceOrigin:MTLOriginMake(src_rect.left, src_rect.top, 0)
             sourceSize:MTLSizeMake(src_rect.right - src_rect.left, src_rect.bottom - src_rect.top, 1)
              toTexture:m_tex
  ...
  const MathUtil::Rectangle<int>& rect, u32 layer, u32 level)
{
  ...
  g_state_tracker->ResolveTexture(static_cast<const Texture*>(src)->GetMTLTexture(), m_tex, layer, level);
}
...
{ @autoreleasepool {
  ...
  id<MTLBlitCommandEncoder> blit = g_state_tracker->GetTextureUploadEncoder();
...
void Metal::Texture::FinishedRendering()
{
  g_state_tracker->UnbindTexture(GetMTLTexture());
}

Metal::StagingTexture::StagingTexture(MRCOwned<id<MTLBuffer>> buffer, StagingTextureType type, const TextureConfig& config)
...
void Metal::StagingTexture::CopyFromTexture(
  const AbstractTexture* src, const MathUtil::Rectangle<int>& src_rect,
...
{ @autoreleasepool {
  ...
  const MTLSize size = MTLSizeMake(src_rect.right - src_rect.left, src_rect.bottom - src_rect.top, 1);
  g_state_tracker->EndRenderPass();
  m_wait_buffer = MRCRetain(g_state_tracker->GetRenderCmdBuf());
  id<MTLBlitCommandEncoder> download = [m_wait_buffer blitCommandEncoder];
  [download setLabel:@"Texture Download"];
  [download copyFromTexture:static_cast<const Texture*>(src)->GetMTLTexture()
                sourceSlice:src_layer
                sourceLevel:src_level
               sourceOrigin:MTLOriginMake(src_rect.left, src_rect.top, 0)
                 sourceSize:size
...
void Metal::StagingTexture::CopyToTexture(
  const MathUtil::Rectangle<int>& src_rect, AbstractTexture* dst,
  const MathUtil::Rectangle<int>& dst_rect, u32 dst_layer, u32 dst_level)
{ @autoreleasepool {
  const size_t stride = m_config.GetStride();
  const u32 offset = dst_rect.top * stride + dst_rect.left * m_texel_size;
  ...
  g_state_tracker->EndRenderPass();
  m_wait_buffer = MRCRetain(g_state_tracker->GetRenderCmdBuf());
  id<MTLBlitCommandEncoder> upload = [m_wait_buffer blitCommandEncoder];
  [upload setLabel:@"Texture Upload"];
  [upload copyFromBuffer:m_buffer
            sourceOffset:offset
...
Metal::Framebuffer::Framebuffer(AbstractTexture* color, AbstractTexture* depth, u32 width, u32 height, u32 layers, u32 samples)
