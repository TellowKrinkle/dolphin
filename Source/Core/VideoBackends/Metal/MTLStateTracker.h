...
public:
  ...
  MTLRenderPassDescriptor* GetRenderPassDescriptor(Framebuffer* framebuffer, MTLLoadAction load_action);

  void EndRenderPass();
  void FlushEncoders();
  void WaitForFlushedEncoders();
  bool HasUnflushedData() { return static_cast<bool>(m_current_render_cmdbuf); }
  bool GPUBusy() { return m_current_draw != 1 + m_last_finished_draw.load(std::memory_order_acquire); }
  void ReloadSamplers();
  ...
  void DispatchComputeShader(u32 groupsize_x, u32 groupsize_y, u32 groupsize_z,
  ...
  {
    assert((m_upload_buffers[static_cast<int>(buffer_idx)].usage.Pos() & static_cast<size_t>(align)) == 0);
...
private:
  ...
  {
    NSString* label;
    id<MTLBuffer> vertex_buffers[2];
    id<MTLBuffer> fragment_buffers[2];
    u32 width;
    u32 height;
  ...
  struct State
  {
    ...
    float sampler_min_lod[8];
    float sampler_max_lod[8];
    SamplerState sampler_states[8];
