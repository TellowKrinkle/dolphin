...
static MTLCompareFunction Convert(CompareMode mode)
{
  ...
  switch (mode)
  {
    ...
    case CompareMode::Less:    return invert_depth ? MTLCompareFunctionGreater      : MTLCompareFunctionLess;
    case CompareMode::Equal:   return MTLCompareFunctionEqual;
    case CompareMode::LEqual:  return invert_depth ? MTLCompareFunctionGreaterEqual : MTLCompareFunctionLessEqual;
    case CompareMode::Greater: return invert_depth ? MTLCompareFunctionLess         : MTLCompareFunctionGreater;
    case CompareMode::NEqual:  return MTLCompareFunctionNotEqual;
    case CompareMode::GEqual:  return invert_depth ? MTLCompareFunctionLessEqual    : MTLCompareFunctionGreaterEqual;
...
static void SetupDSS(MRCOwned<id<MTLDepthStencilState>> (&dss)[Metal::DSSSelector::N_VALUES])
{
  ...
  for (size_t i = 0; i < std::size(dss); i++)
  {
    ...
    [desc setLabel:[NSString stringWithFormat:@"DSS %s%s", to_string(mcompare), sel.UpdateEnable() ? " ZWE" : ""]];
...
static void SetupSamplers(MRCOwned<id<MTLSamplerState>> (&samplers)[Metal::SamplerSelector::N_VALUES])
{
  ...
  for (size_t i = 0; i < std::size(samplers); i++)
  {
    ...
    [desc setLabel:MRCTransfer([[NSString alloc] initWithFormat:@"%s%s%s %s%s%s",
      to_string(sel.MinFilter()), to_string(sel.MagFilter()), to_string(sel.MipFilter()),
      to_string(sel.WrapU()), to_string(sel.WrapV()), sel.AnisotropicFiltering() ? "(AF)" : ""])];
