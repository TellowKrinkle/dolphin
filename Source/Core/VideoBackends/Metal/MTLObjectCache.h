...
struct DSSSelector
{
  ...
  DSSSelector(DepthState state) : DSSSelector(state.updateenable, state.testenable ? state.func : CompareMode::Always) {}
...
struct SamplerSelector
{
  ...
  SamplerSelector(SamplerState state)
  {
    ...
    value |= (static_cast<u32>(state.tm0.wrap_u.Value()) + 3 * static_cast<u32>(state.tm0.wrap_v.Value())) << 4;
  }
...
public:
  ...
  id<MTLDepthStencilState> GetDSS(DSSSelector sel)
  {
    return m_dss[sel.value];
  }
  ...
  id<MTLSamplerState> GetSampler(SamplerState state)
  {
    return GetSampler(SamplerSelector(state));
  }
