...
bool StateTracker::Bind()
{
  ...
  m_dirty_flags &= ~(DIRTY_FLAG_INDEX_BUFFER | DIRTY_FLAG_PIPELINE | DIRTY_FLAG_VIEWPORT |
                     DIRTY_FLAG_SCISSOR);
  return true;
}
...
bool StateTracker::UpdateGXDescriptorSet()
{
      ...
      (m_dirty_flags & DIRTY_FLAG_GX_SSBO || m_gx_descriptor_sets[2] == VK_NULL_HANDLE))
  {
    ...
    writes[num_writes++] = {
        VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr, m_gx_descriptor_sets[2], 1, 0, 1,
        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,      nullptr, &m_bindings.gx_uber_vertex_ssbo, nullptr};
